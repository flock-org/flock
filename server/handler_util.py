...
from storage import AWSStorage, AzureStorage, GCPStorage, LocalStorage
...
MAX_FAULTY_ATTEMPTS = (
    5  # replace this with your desired maximum number of faulty attempts
)
...
def handler_body(event, bucket_name, storage_name, exec_path):
    ...
    if event["authOp"] in ["auth_passcode", "auth_u2f"]:
        ...
        try:
            ...
            last_failed_timestamp = float(
                storage.get_object(f"{op_type}_auth_timestamp")
            )
            ...
            storage.store_object(
                f"{op_type}_auth_timestamp", str(last_failed_timestamp)
            )
    ...
    ### OPERATION SETUP ###
        ...
        if event["op"] == "sharding_setup":
            store_key(
                event["shardingKeyShard"], event["shardingSaltedHash"], storage, result
            )
    ...
    ### OPERATIONS ###
        ...
        if auth_bit == -1:
            ...
            print(
                f"Authentication failed for operation '{op_type}'. Remaining attempts: {n}."
            )
            ...
            storage.store_object(f"{op_type}_auth_counter", str(n))
            print(
                f"Authentication passed for operation '{op_type}'. Counter reset to: {n}."
            )
...
def run_signing_keygen(event, exec_path, storage, result):
    ...
    # with open(
    #     os.path.join(FILES_DIR, "preparams.txt"), "r"
    # ) as f:
    #     preparams = f.read()
    end_time = time.perf_counter()
...
def run_aes_encryption(event, exec_path, storage, result):
    ...
    start_time = time.perf_counter()
    storage.store_object(
        "aes_ciphertext_shard", json.loads(payload)["aes_ctr_out_share"]
    )
    end_time = time.perf_counter()
    result["aes_encrypt_storage_time"] += end_time - start_time
...
def run_freshness_file_server_store(event, storage, result):
    ...
    result["freshness_hashed_content"] = hashlib.sha256(
...
def run_freshness_hash_server_store(event, storage, result):
    ...
    if True:
        if new_file:
            ...
            if event["username"] not in permissions or permissions[
                event["username"]
            ] not in ["owner", "writer"]:
        ...
        end_time = time.perf_counter()
        result["freshness_hash_server_storage_time"] += end_time - start_time
        signature = crypto_util.sign_message(
            event["file_id"] + event["hash_entry"], hash_server_sk
        )
...
def run_freshness_permissions_update(event, storage, result):
    ...
    permissions = string_to_dict(permissions_str)
    if (
        event["username"] not in permissions
        or permissions[event["username"]] != "owner"
    ):
...
def run_store_pir_database(event, storage, result):
    start_time = time.perf_counter()
    serialized_pir_database = pir_util.generate_serialized_elements(
        int(event["num_database_elements"]), event["pir_sk"]
    )
    end_time = time.perf_counter()
...
def auth_u2f_verify_challenge(event, storage, result):
    ...
    is_valid = crypto_util.verify_response(
        vk, challenge, event["authU2fRootHash"], event["authU2fProof"], signature
    )
    end_time = time.perf_counter()
...
def write_to_mmap(data):
    ...
    with open(file_name, "wb") as f:
        total_size = sum(
            4 + len(value.encode("utf-8")) if isinstance(value, str) else 4
