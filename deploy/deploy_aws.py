...
def create_lambda_execution_role(role_name):
    ...
    if not role_exists(role_name):
        try:
            ...
            response = iam.attach_role_policy(PolicyArn=policy_arn, RoleName=role_name)
            time.sleep(10)
            return response
...
def grant_bucket_access(func_name, bucket_name):
    ...
    try:
        ...
        response = iam_client.create_policy(
            PolicyName=policy_name, PolicyDocument=json.dumps(s3_policy)
        )
    except ClientError as e:
    ...
    if function_exists:
        ...
        while True:
            ...
            if last_update_status == "Successful":
                ...
                raise Exception(
                    f"Function update failed: {response.get('LastUpdateStatusReason', '')}"
                )
...
def main(image_uri):
    ...
    response = deploy_lambda_from_docker(
        func_name, image_uri, memory_size=4096, timeout=120
    )
    print(response)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Deploy AWS Lambda from Docker image")
    parser.add_argument(
        "--image_uri", required=True, help="The URI of the Docker image to deploy"
    )

    args = parser.parse_args()
    main(image_uri=args.image_uri)
