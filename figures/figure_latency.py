        ...
        try:
            for mode in ["baseline", "flock"]:
                ...
                with open(os.path.join(folder_path, file), "r") as file:
                    ...
                    print(title, mode)
                    print(
                        "Server time mean and std:",
                        server_time["mean"],
                        server_time["std"],
                    )
                    print(
                        "Client time mean and std:",
                        client_time["mean"],
                        client_time["std"],
                    )
...
if __name__ == "__main__":
    ...
    for op in [
        ...
        latency_files = [
            ...
            and not file.startswith("pir_setup")
        ]
        ...
        latency_dict_baseline = dict(
            sorted(latency_dict_baseline.items(), key=lambda x: int(x[0][2:]))
        )
        latency_dict_flock = dict(
            sorted(latency_dict_flock.items(), key=lambda x: int(x[0][2:]))
        )
        latency_dict_inmem = dict(
            sorted(latency_dict_inmem.items(), key=lambda x: int(x[0][2:]))
        )
        ...
        for idx, latency_data in enumerate(data_list):
            ...
            marker_sizes = [
                6 if colors[idx] == "mediumpurple" else 4 for _ in latencies
            ]
            ax.plot(
                input_sizes,
                latencies,
    ...
    for i in range(3):
        ax_legend.plot(
            [], [], color=colors[i], marker=markers[i], markersize=8, label=labels[i]
        )
