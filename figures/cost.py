...
import client.figures.custom_style as custom_style
from client.figures.custom_style import remove_chart_junk
...
CRYPTO_PRIMITIVES = {
    "secret_recovery": {
        "bandwidth": 25.83,
        "requests_per_min": 1384,
        "storage": 2**10,
        "latency": 0.3404827708378434,
    },
    "signing": {
        "bandwidth": 67.38,
        "requests_per_min": 69,
        "storage": 2**10,
        "latency": 1.3603502629324793,
    },
    "decryption": {
        "bandwidth": 59763,
        "requests_per_min": 5,
        "storage": 2**10 // 8,
        "latency": 21.97429116200656,
    },
    "pir": {
        "bandwidth": 1.38,
        "requests_per_min": 1196,
        "storage": (2**10) * 128,
        "latency": 0.17021635957062245,
    },
    "freshness": {
        "bandwidth": 2.89,
        "requests_per_min": 1169,
        "storage": 2**10,
        "latency": 0.21895868591964246,
    },
}
...
def calculate_average_costs(t):
    ...
    for primitive, data in CRYPTO_PRIMITIVES.items():
        ...
        baseline_cost_cents, _ = calculate_baseline_cost_in_cents(
            bandwidth, requests_per_min, storage
        )
        flock_cost_cents, _ = calculate_flock_cost_in_cents(
            primitive, bandwidth, requests_per_min, storage, latency
        )
...
def avg_network_egress_cost():
    return (
        AWS_NETWORK_EGRESS_COST + AZURE_NETWORK_EGRESS_COST + GCP_NETWORK_EGRESS_COST
    ) / 3
...
def calculate_baseline_cost_in_cents(bandwidth, requests_per_min, storage):
    monthly_cost = (
        AWS_SERVER_MONTHLY_COST + AZURE_SERVER_MONTHLY_COST + GCP_SERVER_MONTHLY_COST
    ) / (30 * 24 * 60 * (t * requests_per_min))
    print(t)
    network_cost = (
        avg_network_egress_cost() * bandwidth / 1000000
    )  # Convert KB to GB for cost calculation
    storage_cost = (
        sum_storage_costs() * storage / 1000000000
...
def calculate_flock_cost_in_cents(
    primitive, bandwidth, requests_per_min, storage, latency
):
    monthly_cost = (AZURE_SERVER_MONTHLY_COST + RELAY_SERVER_MONTHLY_COST) / (
        30 * 24 * 60 * (t * requests_per_min)
    )
    ...
    if primitive in ["freshness", "secret_recovery"]:
        compute_cost = (
            SMALL_AWS_LAMBDA_COST_PER_SECOND + SMALL_GCP_CLOUD_RUN_COST_PER_SECOND
        ) * latency
    else:
        compute_cost = (
            AWS_LAMBDA_COST_PER_SECOND + GCP_CLOUD_RUN_COST_PER_SECOND
        ) * latency
    ...
    total_cost = (
        monthly_cost + compute_cost + network_cost + storage_cost
...
if __name__ == "__main__":
    ...
    for primitive, data in CRYPTO_PRIMITIVES.items():
        ...
        baseline_cost_cents, baseline_compute_only = calculate_baseline_cost_in_cents(
            bandwidth, requests_per_min, storage
        )
        flock_cost_cents, flock_compute_only = calculate_flock_cost_in_cents(
            primitive, bandwidth, requests_per_min, storage, latency
        )
        ratio = (
            flock_cost_cents / baseline_cost_cents if baseline_cost_cents != 0 else 0
        )

        ratios.append(
            ratio
        )  # Storing the numerical value of the ratio for calculations

        # Organizing data in dictionary
        output[primitive] = {
            "storage_cost": format_cost(
                sum_storage_costs() * storage / 1000000000 * 100
            ),  # Convert bytes to GB
            "network_cost": format_cost(
                avg_network_egress_cost() * bandwidth / 1000000 * 100
            ),  # Convert KB to GB
            "baseline": {
    ...
    with open("../figures/crypto_module_costs.json", "w") as f:
    ...
    t = 0.5
    average_baseline_at_specific_t, average_flock_at_specific_t = (
        calculate_average_costs(t)
    )
    print(
        f"Average cost at t={t}: {average_baseline_at_specific_t} for Baseline, {average_flock_at_specific_t} for Flock"
    )
    ...
    custom_style.save_fig(fig, "../figures/cost_graph.png", [3.5, 2.5])
    plt.savefig("../figures/cost_graph.png")
    plt.savefig("../figures/cost_graph.pgf")
    plt.show()
