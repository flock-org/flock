...
normalized_flock_data = [
    flock / baseline if baseline != 0 else 0
    for flock, baseline in zip(flock_data, baseline_data)
]
...
bar1 = ax.bar(
    index, normalized_flock_data, bar_width, label="Flock", color=colors_flock
)
bar2 = ax.bar(
