...
def gen_authshares(bin_string, nP):
    ...
    commands = [
        os.path.join(bin_dir, f"create_authshare_{nP}PC"),
        "gen_authshares",
        deltas_path,
        bin_string,
    ]
...
def generate_salted_hash(password_hex: str) -> str:
    ...
    hash_value = hashlib.pbkdf2_hmac(
        "sha256", bytes.fromhex(password_hex), salt, 100000
    )
...
def check_hashes(hashes: List[str], recovered_key: str) -> bool:
    ...
    salt = hashes[0][:64]
    hash_value = hashlib.pbkdf2_hmac(
        "sha256", bytes.fromhex(recovered_key), bytes.fromhex(salt), 100000
    )
