...
def append_common_fields(datas):
    relay_tag = secrets.token_hex(8)
    for i, data in enumerate(datas):
        data["username"] = USERNAME
        data["routerAddr"] = ROUTER_ADDR
        data["routerPort"] = ROUTER_PORT + (offset - 1) % 4
        data["relayTag"] = relay_tag
        data["numParties"] = 3
...
def invoke(datas):
    ...
    if (
        sys.argv[-1].startswith("user")
        and sys.argv[1] == "baseline"
        and sys.argv[2] in ["sharding_recover"]
    ):
...
def invoke_2PC(datas):
    ...
    if (
        sys.argv[-1].startswith("user")
        and sys.argv[1] == "baseline"
        and sys.argv[2] in ["freshness_retrieve_file", "pir"]
    ):
        URLS[1] = f"https://{AWS_TP_ADDR}:443"
...
def invoke_sharding_setup(log_key_size):
    ...
    sharding_salted_hash = sharding_helper.generated_salted_hash_from_bytes(
        random_bytes
    )
...
def invoke_pir_setup(log_db_size):
    ...
    with open("./output/pir-sk.txt", "r") as f:
        sk_string = f.read()
    datas = [{}, {}]
    for i, data in enumerate(datas):
        data["op"] = "pir_setup"
...
def invoke_freshness_store_file(log_file_size):
    ...
    hash_server_data["hash_entry"] = hashlib.sha256(
    ...
    is_valid = crypto_util.verify_signature(
        file_server_data["file_id"] + hash_server_data["hash_entry"],
        hash_server_sig,
        hash_server_pk,
    )
    if is_valid:
...
def invoke_freshness_retrieve_file():
    ...
    hash_server_pk = crypto_util.vk_from_string(hash_server_pk_str)
    is_valid = crypto_util.verify_signature(
        hash_server_entry, hash_server_sig, hash_server_pk
    )
    if is_valid:
    ...
    if hash_server_entry.startswith(hashed_content):
        print(
            "Hashes are the same, so content is untampered. Here are the latest file contents: ",
            retrieved_content,
        )
...
def compute_u2f_challenges(orig_op, results):
    ...
    with open(f"./output/sk-{USERNAME}.txt", "r") as f:
        sk_string = f.read()
    root_hash, proofs, signature = crypto_util.generate_response(
        list(sorted_challenges.values()), crypto_util.sk_from_string(sk_string)
    )
...
def invoke_auth_u2f():
    ...
    datas = [{}, {}, {}]
    with open("./output/sk-user1.txt", "r") as f:
        sk_string = f.read()
    root_hash, proofs, signature = crypto_util.generate_response(
        list(sorted_challenges.values()), crypto_util.sk_from_string(sk_string)
    )
...
def invoke_one_time_setup():
    datas = [{}, {}, {}]
    for i, data in enumerate(datas):
        data["op"] = "one_time_setup"
    results = invoke(datas)
    return results
...
def gen_pir_requests(indices, database_size):
    serialized_requests_json = exec(
        "gen_requests", exec_path, indices, str(database_size)
    )
...
def bench():
    ...
    elif op == "one_time_setup":
        results = invoke_one_time_setup()
    ...
    for entry in results:
        ...
        if op == "pir":
            extra_time = (
                entry["pir_storage_total_time"]
                + entry["pir_setup_db_time"]
                + entry["pir_write_mem_time"]
            )
