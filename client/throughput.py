...
def send_requests(thread_idx):
    ...
    while True:
        ...
        try:
            if REQUEST_ARGS[1] == "signing_sign":
                result = subprocess.run(
                    ["python3", "invoke.py", *request_args],
                    capture_output=True,
                    timeout=6,
                )
            else:
                result = subprocess.run(
                    ["python3", "invoke.py", *request_args], capture_output=True
                )
...
if __name__ == "__main__":
    ...
    for num_threads in threads:
        ...
        if num_threads in results_dict:
            results_dict[num_threads] = max(results_dict[num_threads], throughput)
        else:
            results_dict[num_threads] = throughput
        # Print results
        print(
            f"Threads: {num_threads} | Total number of requests: {num_requests} | Throughput: {throughput:.2f} requests/minute"
        )
